...
class DualNBackBloc extends Bloc<DualNBackEvent, DualNBackState> {
  ...
  DualNBackBloc() : super(const DualNBackState()) {
    ...
    on<PositionMatchClicked>(_onPositionMatchClicked);
    on<AudioMatchClicked>(_onAudioMatchClicked);
    ...
    // OPRAVA: Reset nyní pouze zruší timer a hodí do menu, NEVYHODNOCUJE HRU!
  ...
  void _onNextRound(NextRound event, Emitter<DualNBackState> emit) {
    ...
    // OPRAVA: Plné dokončení a uložení do paměti se děje POUZE ZDE!
  ...
  void _onPositionMatchClicked(PositionMatchClicked event, Emitter<DualNBackState> emit) {
    if (state.positionMatchClicked || state.currentRound == 0) return;
    ...
    if (state.currentRound <= state.currentN) {
      ...
      if (_positions.last == _positions[nBackIndex]) {
        scoreChange = 1;
      } else {
        pMistake = true;
        scoreChange = -1;
    ...
    if (pMistake) {
      ...
      Future.delayed(const Duration(milliseconds: 300), () {
        if (!isClosed) add(ClearPositionError());
      });
    } else {
  ...
  void _onAudioMatchClicked(AudioMatchClicked event, Emitter<DualNBackState> emit) {
    if (state.audioMatchClicked || state.currentRound == 0) return;
    ...
    if (state.currentRound <= state.currentN) {
      ...
      int nBackIndex = state.currentRound - 1 - state.currentN;
      if (_sounds.last == _sounds[nBackIndex]) {
        scoreChange = 1;
      } else {
        sMistake = true;
        scoreChange = -1;
    ...
    if (sMistake) {
      ...
      Future.delayed(const Duration(milliseconds: 300), () {
        if (!isClosed) add(ClearAudioError());
      });
    } else {
  ...
  void _evaluateRound(Emitter<DualNBackState> emit) {
    ...
    bool pMissed = pMatch && !state.positionMatchClicked;
    bool sMissed = sMatch && !state.audioMatchClicked;
    ...
    if (pMissed || sMissed) {
      ...
      emit(state.copyWith(isPositionError: state.isPositionError || pMissed, isAudioError: state.isAudioError || sMissed));
